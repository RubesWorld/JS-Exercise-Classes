...
class Car {
  ...
  drive(distance){
    let milesAvail = this.tank * this.milesPerGallon;
    let galUsed = Math.floor((milesAvail - distance) / this.milesPerGallon);
    if(milesAvail > distance){
      this.odometer += distance;
      this.tank -= galUsed;
    } else if(milesAvail === distance) {
      this.odometer += distance;
      this.tank = 0;
    } else {
...
class ProjectManager extends Instructor{
  ...
  standUp(channel){
    return `${this.name} announces on ${channel}, @channel standy times!`
  }
